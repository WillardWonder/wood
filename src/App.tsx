...
import { TrendingUp, AlertTriangle, ShieldAlert, FileText, RefreshCw, ExternalLink, Activity } from 'lucide-react';

ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend);

// --- API KEYS (SAFE TO EXPOSE FOR CLIENT-SIDE DASHBOARDS) ---
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // A. FETCH FRED (Hardwood Index WPU081)
        // We use a proxy logic if direct fetch fails, but standard FRED API usually allows read access.
        ...
        const cleanFred = fredJson.observations.reverse().map((o: any) => ({
          date: o.date,
          value: parseFloat(o.value)
        }));
        setFredData(cleanFred);

        // B. FETCH COMTRADE (China Exports)
        // Using Public Preview API (No Key Needed for limited recent data)
        ...
        setError("Live feed connection interrupted. Some charts may show cached baseline.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans p-4 md:p-8">
      ...
      <header className="max-w-7xl mx-auto mb-8 border-b border-red-900/50 pb-4 flex justify-between items-end">
        ...
        <div className="text-right hidden md:block">
          <div className="flex items-center justify-end gap-2 text-green-500 text-xs font-bold">
            <Activity className="animate-pulse" size={12} /> LIVE API FEED
          </div>
          <p className="text-xs text-slate-600 font-mono mt-1">FRED: WPU081 | COMTRADE: HS 4407</p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {error && (
             <div className="bg-red-900/20 border-l-4 border-red-500 p-3 text-red-200 text-sm">
               {error}
             </div>
          )}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-semibold flex items-center gap-2">
                ...
                Market History (Real FRED Data)
              </h2>
            </div>
            <div className="h-[300px]">
              {loading ? <div className="text-center text-slate-500 mt-20">Fetching Federal Reserve Data...</div> : (
                <Line 
                  data={{
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left text-slate-400">
                ...
                <tbody>
                  {comtradeData.length === 0 && !loading ? (
                    <tr><td colSpan={4} className="p-4 text-center">No recent public data available for preview.</td></tr>
